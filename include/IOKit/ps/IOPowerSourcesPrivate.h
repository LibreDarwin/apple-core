...
/*
 * apple-core addition, for PowerManagement's pmconfigd/BatteryTimeRemaining.h,
 * which pmset includes and which uses these types; IOKitUser-100231.120.3,
 * the newest published, predates them.  PowerManagement-1846.120.8.0.1's
 * BatteryTimeRemaining.m reads and writes both through kCFNumberIntType
 * (CFDictionaryGetIntValue/SetIntValue) and loads the state from
 * -unsignedIntValue: unsigned 32-bit values.  The kBHSvcState and kBHSvcFlag
 * constants are left out; nothing pmset compiles uses them.
 */
typedef uint32_t IOPSBatteryHealthServiceState;
typedef uint32_t IOPSBatteryHealthServiceFlags;
