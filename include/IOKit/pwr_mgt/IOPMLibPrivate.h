 ...
 *                  kIOReturnUnsupported if the setting isnÕt supported on this hardware.
 */
...
/*
 * apple-core addition, for PowerManagement's pmset.  IOKitUser-100231.120.3
 * predates this debug assertion type; stock /usr/bin/pmset (macOS 26.5.2,
 * 25F84), which creates it, carries its name as the one string
 * "NoRealPowerSources_debug".
 */
#define kIOPMAssertionTypeDisableRealPowerSources_Debug \
	CFSTR("NoRealPowerSources_debug")

/*
 * apple-core addition, for PowerManagement's pmconfigd/PrivateLib.h, which
 * pmset includes and which declares logASLInactivityWindow() with this
 * type; no published header defines it.  Stock powerd (macOS 26.5.2,
 * 25F84), which compiles that function, compares the 32-bit argument with
 * 0 and logs "immediate" when equal, "largest" otherwise, as PrivateLib.m
 * does against kImmediateInactivityWindow.
 */
typedef enum {
	kImmediateInactivityWindow	= 0,
} inactivityWindowType;
